...
if (!class_exists('Themify_Builder')) :
	...
	public function render_element_ajaxify() {
		...
		$batch=null;
		if ( !empty( $used_gs ) ) {
			$used_gs = array_flip(array_flip($used_gs));
	...
	* Check Builder can edit current post or not
	...
	public function extend_post_types($types) {
	    ...
	    if ($post_types === null) {
			$post_types = array_flip(array_flip(array_merge(
	    ...
	    return array_flip(array_flip(array_merge($types, $post_types)));
	}
	...
	public function save_module_favorite_data() {
		...
		if(isset($_POST['module_state'],$_POST['module_name'])){
			...
			$user_favorite_modules= array_flip(array_flip($user_favorite_modules));
		    }
