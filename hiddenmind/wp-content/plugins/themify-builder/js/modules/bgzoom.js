    ...
    const workingItems=new Set,
        ...
        intersect=(entries, _self)=>{
            ...
            for(let i=entries.length-1;i>-1;--i){
                let item=entries[i],
                    st=item.target.style;
                if(item.target.isConnected){
                    if(item.isIntersecting){
                        item.target.tfOn('transitionend transitioncancel',trEnd,{passive:true,once:true});
                    ...
                    workingItems.delete(item.target);
                    pendingItems.delete(item.target);
                    _self.unobserve(item.target);
                }
            }
