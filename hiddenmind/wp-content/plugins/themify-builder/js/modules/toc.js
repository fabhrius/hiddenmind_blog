...
((Themify,doc,und)=>{
    ...
    const obServer= new IntersectionObserver((entity,self)=>{
            for(let i=entity.length-1;i>-1;--i){
    ...
    click=e=>{
        let target = e.target?.closest('.tf_fa,a');
        if(target){
            if(target.tagName==='A'){
        ...
        if(in_tags){
            ...
            if(container?.length>0){
                    ...
                    draw=(items,level)=>{
                        ...
                        for(let i=0,len=items.length;i<len;++i){
                            ...
                            if(item.childs.length>0){
                                ...
                                f2.appendChild(draw(item.childs,level));
                            }
                            li.appendChild(f2);
                            fr.appendChild(li);
                        }
                ...
                for(let i=0,len=container.length;i<len;++i){
                    ...
                    for(let j=0,len2=tags.length;j<len2;++j){
                        let tag=tags[j];
                        if(!allTags.has(tag) && !tag.closest(exclude) && (!ex_tags || (!tag.matches(ex_tags) && !tag.closest(ex_tags)))){
                            allTags.add(tag);
                            if(tag.offsetWidth>0 && tag.offsetHeight>0 && tag.getClientRects().length>0 ){
                                let textContent=tag.textContent.trim();
                                if(textContent){
                                    if(maxt>0 && textContent.length>maxt){
                                        textContent=textContent.substr(0,maxt);
                                    }
                                    let hasId=tag.dataset.isSet?null:tag.id,
                                        id=hasId || textContent.toLowerCase();
                                    id=id.replace(/(\r\n|\n|\r)/gm,' ').replace(/[`’~!@#$%^&*()|+\=?;:'",.<>\{\}\[\]\\\/]/gi, '').replace(/\s\s+/gu, ' ');
                                    if(!hasId){
                                        if(Themify.is_builder_active===true){
                                            tag.dataset.isSet=true;
                                        }
                                        id=id.replace(/\s/gu, '-');
                                        if(id.length>maxLength){
                                            id=id.split('-');
                                            if(id.length>1){
                                                for(let k=id.length-1;k>-1;--k){
                                                    id.splice(k,1);
                                    ...
                                    tag.id=id;
                                    tag.classList.add('tb_toc_el');
                                    arr.push({
                                       id:encodeURIComponent(id),
                                       text:textContent,
                                       h:+tag.nodeName.replace('H',''),
                                       childs:[]
                                    });
                                    if(Themify.is_builder_active!==true){
                ...
                if(arr.length>min){
                    ...
                    requestAnimationFrame(()=>{
                        ...
                        if(Themify.is_builder_active===false && hash !== '' && hash!=='#'){
                            el.querySelector('a[href="#'+hash.replace('#','')+'"]')?.click();
                        }
                    });
                }
                else{
    ...
    Themify.on('builder_load_module_partial tb_toc', (el,isLazy)=>{
        ...
        for(let items = Themify.selectWithParent('module-toc',el),i=items.length-1;i>-1;--i){
            init(items[i]);
        }
    }).on('infiniteloaded',()=>{
    ...
    .on('tfsmartresize',e=>{
        requestAnimationFrame(()=>{
            ...
            if(bp!==prevBp){
                prevBp=bp;
                for(let toc=doc.tfClass('tb_toc_lv_1'),i=toc.length-1;i>-1;--i){
