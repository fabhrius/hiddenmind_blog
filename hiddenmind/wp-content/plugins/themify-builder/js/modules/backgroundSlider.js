...
(($,Themify,vars) =>{
	...
	vars.autoplay=vars.backgroundSlider?.autoplay?parseInt(vars.backgroundSlider.autoplay, 10):5000;
    const  _init= items=> {
        if (vars.autoplay <= 10) {
            vars.autoplay *= 1000;
        }
        for(let i=items.length-1;i>-1;--i){
            ...
            $backel.tb_backstretch(rsImages, {
                speed: parseInt(item.dataset.sliderspeed),
                duration: vars.autoplay,
                mode: item.dataset.bgmode
            });
            ...
            childs=item.querySelectorAll('.row-slider-prev,.row-slider-next,.row-slider-dot');
            ...
            if (item.dataset.bgmode === 'kenburns-effect') {
                let lastIndex,
                    kenburnsActive = 0,
