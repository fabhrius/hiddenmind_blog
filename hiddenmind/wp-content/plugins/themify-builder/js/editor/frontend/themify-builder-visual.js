((Themify, win, topWindow, doc, api,und) => {
    ...
    api.EdgeDrag = {
        ...
        async optionsClick(e) {
            e.stopPropagation();
            const isInlineEdit= e.target.closest('.tb_dragger_value')!==null;
            if(!isInlineEdit){
                this.addEdgesOptions(e.target.closest('.tb_dragger_options'));
            }
            const target = e.target.nodeName === 'LI' ||isInlineEdit? e.target : e.target.parentNode;
            if (target.nodeName === 'LI') {
                const isUnit = !target.classList.contains('current') && target.parentNode.classList.contains('tb_dragger_units'),
                    ...
                    actionBar=api.ActionBar;
                actionBar.disable = true;
                if (isUnit || isType) {
                    api.undoManager.start('style');
                    if (isUnit) {
                        await this.changeUnit(target);
                    } else {
                        await this.changeApply(target);
                    }
                    api.undoManager.end('style');
                    actionBar.disable = null;
                }
            }else if(isInlineEdit && !target.classList.contains('tb_disable_sorting')){
               this.editValue(target);
            }
        },
        editValue(target){
            target.classList.add('tb_disable_sorting');
            let el=target.closest('.tb_dragger'),
                unit=el.dataset.u,
                unitText=doc.createElement('span'),
                input=doc.createElement('input'),
                prop=el.classList.contains('tb_dragger_padding') ? 'padding' : 'margin',
                oldV=el.dataset.v || 0,
                {min,max}=this.getMinMax(prop,unit),
                baseEl=el.closest('[data-cid]'),
                enter=e=>{
                    e.stopPropagation();
                    let v=input.value;
                    if(v!==''){
                        let parsedV=v.replaceAll(',','.').replace(/[^\d.-]/g, '');
                        if(min>=0){
                            parsedV=parsedV.replaceAll('-','');
                        }
                        parsedV=unit==='em'?parseFloat(parsedV):parseInt(parsedV);
                        if(isNaN(parsedV)){
                            parsedV='';
                        }
                        else if(parsedV<min){
                ...
                onChange=async e=>{
                    e.stopPropagation();
                    let newV=input.value.replaceAll(',','.');
                        newV=unit==='em'?parseFloat(newV):parseInt(newV);
                    ...
                    input.tfOff('focusout',onChange,{passive:true,once:true})
                        .tfOff('keyup',enter,{passive:true}).remove();
                    unitText.remove();
                    target.textContent=newV+unit;
                    if(newV.toString()!==oldV){
                        api.ActionBar.disablePosition=true;
                        el.dataset.v=newV;
                        target.dataset.unit=unit;
                        const isVisible=el.matches(':hover') || getComputedStyle(el).opacity!=0,
                            model=api.Registry.get(baseEl.dataset.cid),
                            isSame=model && api.activeModel?.id === model.id,
                            trEnd=()=>{
                                if(isVisible){
                                    baseEl.tfOff('transitionend transitioncancel',trEnd,{passive:true,once:true}).style.transition='';
                                    this.setModulePosition(el);
                                }
                                setTimeout(()=>{
                                    el=unit=unitText=enter=onChange=baseEl=input=null;
                                },500);
                                api.ActionBar.disablePosition=null;
                            };
                        doc.body.classList.add('tb_edge_drag_start');
                        el.tfClass('tb_dragger_lightbox')[0]?.remove();
                        this.addEdgesOptions(el);
                        if(isSame===false){
                            api.undoManager.start('style');
                        }
                        if(isVisible){
                            baseEl.tfOn('transitionend transitioncancel',trEnd,{passive:true,once:true}).style.transition=prop+' .1s ease';
                        }
                        this.changeUnit(target);
                        if(isSame===false){
                            api.undoManager.end('style');
                        }
                        target.removeAttribute('data-unit');
                        el.tfClass('tb_dragger_lightbox')[0]?.remove();
                        baseEl.classList.remove('tb_dragger_input_mode');
                    ...
                    target.classList.remove('tb_disable_sorting');
                };
            input.type='number';
            input.min=min;
            input.max=max;
            input.step=unit==='em'?0.01:1;
            input.setAttribute('inputmode',(unit==='em'?'decimal':'numeric'));
            input.tfOn('focusout',onChange,{passive:true,once:true})
            .tfOn('keyup',enter,{passive:true}).value=oldV;

            unitText.textContent=unit;
            el.tfClass('tb_dragger_lightbox')[0]?.remove();
            target.innerHTML='';
            target.append(input,unitText);
            api.SmallPanel.hide();
            baseEl.classList.add('tb_dragger_input_mode');
            input.focus();
        },
        changeUnit(el) {
            return new Promise(resolve => {
                const   edge = el.closest('.tb_dragger'),
                        ...
                        u = el.dataset.unit || el.textContent || 'px',
                        v = edge.dataset.v,
                ...
                let apply = edge.tfClass('tb_dragger_types')[0],
                        ...
                        model = api.Registry.get(item.dataset.cid),
                        elType = model.get('mod_name'),
                        selector = ThemifyStyles.getBaseSelector(elType, model.get('element_id'));
                if (apply) {
        ...
        convert(el, prevU, u, v) {
            ...
            const emSize = u === 'em' || prevU === 'em' ? parseFloat(getComputedStyle(p).fontSize) : null,
                ...
                {min,max}=this.getMinMax((el.classList.contains('tb_dragger_padding')?'padding':'margin'),u);
            if (prevU === 'px') {
                if (u === 'em') {
        ...
        getMinMax(prop,unit){
            const units=ThemifyConstructor.getMarginPaddingUnits(prop,unit),
                min=units.min || 0,
                max=units.max || 100;
        ...
        getCurrentStyling(id, model, type) {
            ...
            if (api.activeModel?.id === model.id) {
            ...
            if (v !== und && v !== '' && model.type === 'column' && v.toString().includes(',')  && (id.indexOf('padding') === 0 || id.includes('margin'))) {
                v = v.trim().split(',');
                v = v[1] !== und && v[1] !== ''?v[1].trim():v[0].trim();
            }
            return v;
        },
            ...
            if (e.which === 1) {
                ...
                if (model) {
                    let items = [],
                            ...
                            u = el.dataset.u || 'px', 
                            getSpeed = (x, y,type) => {
								let distance =type==='left' || type==='right'?Math.abs(x - speedX):Math.abs(y - speedY),
                                    speed = u==='%'?1:Math.round(distance);
								if (speed <= 0) {
									speed = u === 'px' || u === '%' ? 1 : .1;
								} else if (u === 'em'){
                                    speed /= 10;
                            ...
                            _start = e => {
                                ...
                                el.tfClass('tb_dragger_lightbox')[0]?.remove();
                                isSame = api.activeModel?.id === model.id;
                                if (isSame === false) {
                                    api.undoManager.start('style');
                                }
                                ...
                                const tmp = el.parentNode.children,
                                    units=self.getMinMax(baseProp,u);
                                min=units.min || 0;
                                max=units.max || 100;
                                for (let i = tmp.length - 1; i > -1; --i) {
							...
							_up=function(e){
								...
								this.style.willChange = module.style.willChange = '';
								...
								if (isStart) {
									...
									requestAnimationFrame(() => {
										...
										for (let i = items.length - 1; i > -1; --i) {
                                            ...
                                            item.tfClass('tb_dragger_lightbox')[0]?.remove();
											if (index === false || !cssRules[index]) {
										...
										api.ActionBar.disable = apply = module = timer = isSame = items = current = prevY = prevX=min=max=isStart=null;
										self.addEdges(model);
									});
								}
								else{
                    ...
                    module.style.willChange = baseProp;
