...
((api, Themify, doc) => {
    ...
    TB_Layouts ={
        ...
        loadLayout(target) {
            ...
            const box = target.closest('ul').getBoundingClientRect(),
                promises=[];
            for(let [_i,item] of LayoutList.instanes){
    ...
    class LayoutList{
        ...
        search(el){
            ...
            for (let i = list.length - 1; i > -1; --i) {
                list[i].closest('li').style.display = (s === '' || list[i].textContent.toUpperCase().includes(s)) ? '' : 'none';
            }
        }
        clearSearch(el){
            ...
            if (!el.classList.contains('current')) {
                ...
                for (let i = list.length - 1; i > -1; --i) {
                    ...
                    if (!show) {
                        ...
                        if (cat) {
                            show = text === cat || (selector === '*' && cat.includes(text));
                        }
                    }
        ...
        createItems(items,selectedCat){
            const ext = webp ? 'webp' : 'png',
                ...
                placeholder = 'https://via.placeholder.com/' + imgW + 'x' + imgH + '.' + ext + '?text=',
                categories=new Set,
    ...
    class Predesigned extends LayoutList{
        ...
        getList(){
            return Themify.fetch('', null, fetchArgs, themifyBuilder.paths[this.id].url);
        }
        async getItem(slug){
            const file = themifyBuilder.paths[this.id].single.replace('{SLUG}', slug);
            try{
                let res=await Themify.fetch('', null, fetchArgs, file),
                    data = JSON.stringify( res );
                if ( data.includes( api.GS.key )) {
                    data = { builder_data : JSON.parse( data ) };
                    try{
                        data.used_gs=await Themify.fetch('', null, fetchArgs, themifyBuilder.paths[this.id].single.replace('{SLUG}', slug + '-gs'));
                    }
                    catch(e){
