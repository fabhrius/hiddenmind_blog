			...
			array(
				...
				'options' => array(
					array(
						...
						'mode' => 'sprite',
						'options' => array(
							array('img' => 'normall_button', 'value' => 'normal', 'label' => 'n'),
					...
					array(
						...
						'options' => array(
							...
							array('img' => 'none', 'value' => 'none', 'label' => 'none')
						)
					),
					array(
						'id' => 'icon_arrangement',
						...
						'mode' => 'sprite',
						'options' => array(
							array('img' => 'horizontal_button', 'value' => 'icon_horizontal', 'label' => 'hrztal'),
							array('img' => 'vertical_button', 'value' => 'icon_vertical', 'label' => 'vertical'),
						)
					),
					array(
						'id' => 'icon_position',
				...
				self::get_tab(array(
					'n' => array(
						'options' => array(
							...
							self::get_font_size(array(' em', ' a', ' span')),
							self::get_line_height(array(' em', ' a', ' span')),
					...
					'h' => array(
						'options' => array(
							...
							self::get_font_size(array(':hover em', ':hover a', ':hover span'), 'f_s_h', '', ''),
	...
	protected function _visual_template() {
		...
		<?php echo $module_args['before_title']; ?>{{{ data.mod_title_icon }}}<?php echo $module_args['after_title']; ?>
			...
			for(let i=0;i<arr.length;++i){
				...
				w=item.w_i || '',
				h=item.h_i || '',
				icon=t!='image'?(t!='l'?(item.icon ? api.Helper.getIcon(item.icon).outerHTML:'') : (api.Helper.getLottie(item,'parent').outerHTML || '')):item.image;
				if ( item.bg ) {
				...
				<div class="module-icon-item">
					...
					<a href="{{ item.link }}"{{ link_target }}{{{ link_lightbox_class }}}{{ lightbox_data }}>
						<# }
						if (t=='image' && icon){ #>
							<img class="tf_box {{ color_bg }}" src="{{ icon }}" data-no-update data-repeat="content_icon" data-w="w_i" data-h="h_i" width="{{w}}" height="{{h}}" data-name="image">
						<# }
						else if (t!='image' && icon){ #>
							<em class="tf_box {{ color_bg }}" style="{{ style }}"><# print(icon)#></em>
						<# }
