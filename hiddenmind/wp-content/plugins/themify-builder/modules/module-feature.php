...
class TB_Feature_Module extends Themify_Builder_Component_Module {
					...
					array(
						...
						'options' => array(
							...
							array(
								...
								'binding' => array(
									'small' => array('hide' => 'custom_circle_size_feature'),
									'medium' => array('hide' => 'custom_circle_size_feature'),
									'large' => array('hide' => 'custom_circle_size_feature'),
									'custom' => array('show' => 'custom_circle_size_feature'),
								)
							),
							array(
	...
	protected function _visual_template() {
		...
		let w=args.circle_stroke_feature || 0,
			...
			type=args.icon_type_feature,
			subtype=args.stype || 'i',
			insetColor=args.icon_bg_feature!=='' && type!='image_icon'?api.Helper.toRGBA(args.icon_bg_feature):'',
			icon=type!='image_icon'?(subtype!='l'?(args.icon_feature ? api.Helper.getIcon(args.icon_feature).outerHTML:'') : (api.Helper.getLottie(args,'parent').outerHTML || '')):'',
				...
				<a href="{{ args.link_feature }}">
					...
					<span class="module-feature-chart-html5 tf_box tf_rel tf_inline_b"<# if(st!==''){#> style="{{st}}"<#}#>>
						...
						<span class="chart-html5-circle tf_w tf_h">
							<# if(type=='icon' && icon){#>
								<em class="module-feature-icon tf_rel"<# if(color!=''){ #> style="color:<# print(color) #>"<# } #>><# print(icon)#></em>
							<# 
							}
						...
						<# if ( type == 'both' && icon ) {
							let style = ( color != '' ? 'color:' + color + ';' : '' ) + ( insetColor != '' ? 'background-color:' + insetColor: '' );
						#>
							<span class="module-feature-icon-wrap tf_abs" style="transform:rotate({{pos}}deg)">
								<span class="module-feature-icon tf_inline_b" style="transform:translateY(-50%) rotate(-{{pos}}deg);{{style}}"><# print( icon ) #></span>
							</span>
						<# } #>
					</span>
					<# if(args.link_feature){ #>
