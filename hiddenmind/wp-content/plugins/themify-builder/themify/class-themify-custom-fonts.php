...
class Themify_Custom_Fonts {
	...
	private static function get_variants( $variants ) {
		...
		if ( !empty( $variants ) && is_array( $variants ) ) {
			...
			$vars = array_values( array_flip( array_flip( $vars ) ));
		}
		return $vars;
	}
	...
	private static function get_font_face_css( $fonts, $builder=false ) {
		...
		if(true===$builder){
			...
			$cf[$f[0]]=!isset($cf[$f[0]])?$v:array_flip( array_flip(array_merge($cf[$f[0]],$v)));
		    }
		    $customizer_fonts=$cf;
		    $cf=null;
		}
		foreach ( $fonts as $font ) {
			...
			$api_fonts[ $font_family ] = !isset($api_fonts[ $font_family ])?$variations:array_flip( array_flip(array_merge($api_fonts[ $font_family ],$variations)));
		    }
		}
		unset($fonts);
		if ( !empty( $api_fonts ) ) {
