...
class Themify_Twitter_Api {
	...
	public function query( $query, $settings = [] ) {
		...
		$settings = wp_parse_args( $settings, [
			...
			'twitter_keys_page' => admin_url( 'admin.php?page=themify#setting-integration-api' ),
		] );
		...
		$query_string = 'screen_name='. urlencode( strip_tags( sanitize_user( str_replace( '@', '', $query['username'] ) ) ) ) . '&count=' . $query['limit'] . '&include_rts='. (int) $query['include_retweets'] . '&exclude_replies=' . (int) $query['exclude_replies'] . '&include_entities=true';
	...
	public static function clear_cache() {
	    ...
	    return Themify_Storage::deleteByPrefix('tf_twit_');
	}
