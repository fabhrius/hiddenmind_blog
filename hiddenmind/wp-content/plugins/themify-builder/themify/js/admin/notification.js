    ...
    hover=function(){
        if(isHover!==true && isVisible===true){
            ...
            this.tfOn('pointerleave',e=>{
                Themify.triggerEvent(e.currentTarget,e.type);
        ...
        async show(type,text,duration){
            ...
            return new Promise(async resolve=>{
                const notify=this.el.shadowRoot.querySelector('.'+type),
                    setMsg=msg=>{
                        if(text instanceof DocumentFragment || text instanceof HTMLElement || text instanceof Document ){
                            msg.innerHTML='';
            ...
            return new Promise(async resolve=>{
                ...
                leave=function(){
                    if(isHover===true){
                        ...
                        this.tfOff('pointerenter',hover,{passive:true})
                        .tfOff('transitioncancel transitionend',end, {passive: true,once: true})
                        .tfOn('transitionend transitioncancel', end, {passive: true,once: true})
                        .classList.remove('show');
                    }
                };
                if(isVisible===true){
                    if(isHover===false || !notify.matches(':hover')){
                        isHover=true;
                        leave.call(notify);
                    }
                    else{
